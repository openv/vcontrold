...
Aufruf:
vclient -h <ip:port> [-c <command1,command2,..>] [-f <commandfile>] [-s <csv-Datei>] [-v]

-h : IP:Port des vcontrold
-c: Liste von auszuführenden Kommandos, durch Komma getrennt
...
-t: TemplateFile
-o: outfile
-x: execute File: das mit -t angegebene Template wird ersetzt und direkt uasgefuehrt.
...
Damt ist es einfach möglich, die Messwerte der Heizung auszulesen und direkt in eine Datenbank zu schreiben.
...
Wir der Schalter -o <datei> angegeben, so wird das Ergebnis in die angegebene Datei geschrieben.
Wird der Schalter -x <datei> verwendet, wird das Ergebnis in die Datei geschrieben und diese direkt ausgefuehrt.
