...
char *getPropertyNode(xmlAttrPtr cur,xmlChar *name) {
	if ((cur) && 
	    (cur->type == XML_ATTRIBUTE_NODE) &&
	     strstr(cur->name,name)) 
		return(getTextNode((xmlNodePtr) cur));
	else if (cur && cur->next) 
		return(getPropertyNode(cur->next,name));
	else
		return(NULL);
}
...
configPtr parseConfig(xmlNodePtr cur) {
	...
	cfgPtr->aPtr=NULL;
	while(cur) {
		sprintf(string,"CONFIG:(%d) Node::Name=%s Type:%d Content=%s",cur->line,cur->name,cur->type,cur->content);
		...
		if (strstr((char *)cur->name,"serial"))  {
		...
		else if (strstr(cur->name,"net"))  {
			netFound=1;
			prevPtr=cur;
			cur=cur->children;
		}
		else if (strstr(cur->name,"logging"))  {
			logFound=1;
			prevPtr=cur;
			cur=cur->children;
		}
		else if (strstr(cur->name,"device"))  {
			chrPtr=getPropertyNode(cur->properties,(xmlChar *)"ID");
			sprintf(string,"     Device ID=%s",cfgPtr->devID);
			logIT(LOG_INFO,string);
			if (chrPtr) {
		...
		else if (serialFound && strstr(cur->name,"tty"))  {
		...
		else if (netFound && strstr(cur->name,"port"))  {
		 	chrPtr=getTextNode(cur);	
			sprintf(string,"   (%d) Node::Name=%s Type:%d Content=%s",cur->line,cur->name,cur->type,chrPtr);
			logIT(LOG_INFO,string);
			if (chrPtr) {
		...
		else if (netFound && strstr(cur->name,"allow"))  {
			...
			if(inet_addr(ip) != INADDR_NONE) {
				...
				if(!cfgPtr->aPtr) 
					cfgPtr->aPtr=aPtr;
				sprintf(string,"     Allow IP:%s Size:/%d",ip,size);
				logIT(LOG_INFO,string);
			}
		...
		else if (logFound && strstr(cur->name,"file"))  {
		 	chrPtr=getTextNode(cur);	
			sprintf(string,"   (%d) Node::Name=%s Type:%d Content=%s",cur->line,cur->name,cur->type,chrPtr);
			logIT(LOG_INFO,string);
			if (chrPtr) {
		...
		else if (logFound && strstr(cur->name,"syslog"))  {
		 	chrPtr=getTextNode(cur);	
			sprintf(string,"   (%d) Node::Name=%s Type:%d Content=%s",cur->line,cur->name,cur->type,chrPtr);
			logIT(LOG_INFO,string);
		...
		else if (logFound && strstr(cur->name,"debug"))  {
		 	chrPtr=getTextNode(cur);	
...
unitPtr parseUnit(xmlNodePtr cur) {
	...
	while(cur) {
		sprintf(string,"UNIT: (%d) Node::Name=%s Type:%d Content=%s",cur->line,cur->name,cur->type,cur->content);
		logIT(LOG_INFO,string);
		if (cur->type == XML_TEXT_NODE) {
			cur=cur->next;
			continue;
		}
		if (strstr(cur->name,"unit"))  {
			unit=getPropertyNode(cur->properties,(xmlChar *)"name");
			if (unit) { /* neue Unit gelesen */
				sprintf(string,"Neue Unit: %s",unit);
				logIT(LOG_INFO,string);
		...
		else if (unitFound && strstr(cur->name,"enum"))  {
			chrPtr=getPropertyNode(cur->properties,(xmlChar *)"text");
			sprintf(string,"   (%d) Node::Name=%s Type:%d Content=%s (text)",cur->line,cur->name,cur->type,chrPtr);
			logIT(LOG_INFO,string);
			if (chrPtr) {
				...
				if (chrPtr) {
					sprintf(string,"          (%d) Node::Name=%s Type:%d Content=%s (bytes)",cur->line,cur->name,cur->type,chrPtr);
					logIT(LOG_INFO,string);
					bzero(string,sizeof(string));
		...
		else if (unitFound && strstr(cur->name,"abbrev"))  {
			chrPtr=getTextNode(cur);	
			sprintf(string,"   (%d) Node::Name=%s Type:%d Content=%s",cur->line,cur->name,cur->type,chrPtr);
			logIT(LOG_INFO,string);
			if (chrPtr) {
		...
		else if (unitFound && (strcmp(cur->name,"calc")==0))  {
			chrPtr=getPropertyNode(cur->properties,(xmlChar *)"get");
			sprintf(string,"   (%d) Node::Name=%s Type:%d Content=%s (get)",cur->line,cur->name,cur->type,chrPtr);
			logIT(LOG_INFO,string);
			if (chrPtr) {
			...
			sprintf(string,"   (%d) Node::Name=%s Type:%d Content=%s (set)",cur->line,cur->name,cur->type,chrPtr);
			logIT(LOG_INFO,string);
			if (chrPtr) {
		...
		else if (unitFound && (strcmp(cur->name,"icalc")==0))  {
			chrPtr=getPropertyNode(cur->properties,(xmlChar *)"get");
			sprintf(string,"   (%d) Node::Name=%s Type:%d Content=%s (get)",cur->line,cur->name,cur->type,chrPtr);
			logIT(LOG_INFO,string);
			if (chrPtr) {
		...
		else if (unitFound && strstr(cur->name,"type"))  {
			chrPtr=getTextNode(cur);	
			sprintf(string,"   (%d) Node::Name=%s Type:%d Content=%s",cur->line,cur->name,cur->type,chrPtr);
			logIT(LOG_INFO,string);
			if (chrPtr) {
		...
		else if (unitFound && strstr(cur->name,"entity"))  {
			chrPtr=getTextNode(cur);	
			sprintf(string,"   (%d) Node::Name=%s Type:%d Content=%s",cur->line,cur->name,cur->type,chrPtr);
			logIT(LOG_INFO,string);
			if(chrPtr) {
...
macroPtr parseMacro(xmlNodePtr cur) {
	...
	while(cur) {
		sprintf(string,"MACRO: (%d) Node::Name=%s Type:%d Content=%s",cur->line,cur->name,cur->type,cur->content);
		logIT(LOG_INFO,string);
		if (cur->type == XML_TEXT_NODE) {
			cur=cur->next;
			continue;
		}
		if (strstr(cur->name,"macro"))  {
			macro=getPropertyNode(cur->properties,(xmlChar *)"name");
			if (macro) { /* neues Macro gelesen */
				sprintf(string,"Neues Macro: %s",macro);
				logIT(LOG_INFO,string);
		...
		else if (macroFound && strstr(cur->name,"command"))  {
			chrPtr=getTextNode(cur);	
			sprintf(string,"   (%d) Node::Name=%s Type:%d Content=%s",cur->line,cur->name,cur->type,chrPtr);
			logIT(LOG_INFO,string);
			if (chrPtr) { 
			...
			sprintf(string,"Fehler beim parsen macro");
			logIT(LOG_INFO,string);
			return(NULL);
		}
	}
...
commandPtr parseCommand(xmlNodePtr cur,commandPtr cPtr,devicePtr dePtr) {
	...
	while(cur) {
		sprintf(string,"COMMAND: (%d) Node::Name=%s Type:%d Content=%s",cur->line,cur->name,cur->type,cur->content);
		logIT(LOG_INFO,string);
		if (xmlIsBlankNode(cur))  {
			cur=cur->next;
			continue;
		}
		if (strcmp(cur->name,"command")==0)  {
			...
			if (command) { /* neues Command gelesen */
				sprintf(string,"Neues Command: %s",command);
				logIT(LOG_INFO,string);
		...
		else if (commandFound && strstr(cur->name,"addr"))  {
			chrPtr=getTextNode(cur);	
			sprintf(string,"   (%d) Node::Name=%s Type:%d Content=%s",cur->line,cur->name,cur->type,chrPtr);
			logIT(LOG_INFO,string);
			if (chrPtr) {
		...
		else if (commandFound && strstr(cur->name,"error"))  {
			chrPtr=getTextNode(cur);	
			sprintf(string,"   (%d) Node::Name=%s Type:%d Content=%s",cur->line,cur->name,cur->type,chrPtr);
			logIT(LOG_INFO,string);
			if (chrPtr) {
		...
		else if (commandFound && strstr(cur->name,"unit"))  {
			chrPtr=getTextNode(cur);	
			sprintf(string,"   (%d) Node::Name=%s Type:%d Content=%s",cur->line,cur->name,cur->type,chrPtr);
			logIT(LOG_INFO,string);
			if (chrPtr) {
		...
		else if (commandFound && (strcmp(cur->name,"precommand")==0))  {
			chrPtr=getTextNode(cur);	
			sprintf(string,"   (%d) Node::Name=%s Type:%d Content=%s",cur->line,cur->name,cur->type,chrPtr);
			logIT(LOG_INFO,string);
			if (chrPtr) {
		...
		else if (commandFound && strstr(cur->name,"description"))  {
			chrPtr=getTextNode(cur);	
			sprintf(string,"   (%d) Node::Name=%s Type:%d Content=%s",cur->line,cur->name,cur->type,chrPtr);
			logIT(LOG_INFO,string);
			if (chrPtr) {
		...
		else if (commandFound && strstr(cur->name,"len"))  {
			chrPtr=getTextNode(cur);	
			sprintf(string,"   (%d) Node::Name=%s Type:%d Content=%s",cur->line,cur->name,cur->type,chrPtr);
			logIT(LOG_INFO,string);
			if (chrPtr) {
		...
		else if (commandFound && strstr(cur->name,"bit"))  {
			chrPtr=getTextNode(cur);	
			sprintf(string,"   (%d) Node::Name=%s Type:%d Content=%s",cur->line,cur->name,cur->type,chrPtr);
			logIT(LOG_INFO,string);
			if (chrPtr) {
...
icmdPtr parseICmd(xmlNodePtr cur) {
	...
	while(cur) {
		sprintf(string,"ICMD: (%d) Node::Name=%s Type:%d Content=%s",cur->line,cur->name,cur->type,cur->content);
		logIT(LOG_INFO,string);
		if (xmlIsBlankNode(cur))  {
		/* if (cur->type == XML_TEXT_NODE) { */
			cur=cur->next;
		...
		if (strstr(cur->name,"command"))  {
			command=getPropertyNode(cur->properties,(xmlChar *)"name");
			if (command) { /* neues Command gelesen */
				sprintf(string,"Neues iCommand: %s",command);
				logIT(LOG_INFO,string);
		...
		else if (commandFound && strstr(cur->name,"send"))  {
			chrPtr=getTextNode(cur);	
			sprintf(string,"   (%d) Node::Name=%s Type:%d Content=%s",cur->line,cur->name,cur->type,chrPtr);
			logIT(LOG_INFO,string);
			if (chrPtr) { 
		...
		else if (commandFound && strstr(cur->name,"retry"))  {
			chrPtr=getTextNode(cur);	
			sprintf(string,"   (%d) Node::Name=%s Type:%d Content=%s",cur->line,cur->name,cur->type,chrPtr);
			logIT(LOG_INFO,string);
			if (chrPtr) { 
		...
		else if (commandFound && strstr(cur->name,"recvTimeout"))  {
			chrPtr=getTextNode(cur);	
			sprintf(string,"   (%d) Node::Name=%s Type:%d Content=%s",cur->line,cur->name,cur->type,chrPtr);
			logIT(LOG_INFO,string);
			if (chrPtr) { 
...
devicePtr parseDevice(xmlNodePtr cur,protocolPtr pPtr) {
	...
	while(cur) {
		sprintf(string,"DEVICE: (%d) Node::Name=%s Type:%d Content=%s",cur->line,cur->name,cur->type,cur->content);
		logIT(LOG_INFO,string);
		if (cur->type == XML_TEXT_NODE) {
			cur=cur->next;
			continue;
		...
		if (strstr((char *)cur->name,"device"))  {
			...
			if (proto) { /* neues Protocol gelesen */
				sprintf(string,"    Neues Device: name=%s ID=%s proto=%s",name,id,proto);
				logIT(LOG_INFO,string);
				...
				if (!(dPtr->protoPtr=getProtocolNode(pPtr,proto))) {
					sprintf(string,"Protokoll %s nicht definiert",proto);
					logIT(LOG_ERR,string);
					return(NULL);
				}
...
protocolPtr parseProtocol(xmlNodePtr cur) {
	...
	while(cur) {
		sprintf(string,"PROT: (%d) Node::Name=%s Type:%d Content=%s",cur->line,cur->name,cur->type,cur->content);
		logIT(LOG_INFO,string);
		if (cur->type == XML_TEXT_NODE) {
			cur=cur->next;
			continue;
		...
		if (strstr((char *)cur->name,"protocol"))  {
			...
			if (proto) { /* neues Protocol gelesen */
				sprintf(string,"Neues Protokoll: %s",proto);
				logIT(LOG_INFO,string);
		...
		else if (protoFound && strstr(cur->name,"commands"))  {
...
int parseXMLFile(char *filename) {
	...
	if (ns == NULL) {
       		sprintf(string,"document of the wrong type, vcontrol Namespace not found");
		logIT(LOG_ERR,string);
        	xmlFreeDoc(doc);
        	return(0);
	}
	if (xmlStrcmp(cur->name, (const xmlChar *) "V-Control")) {
		sprintf(string,"document of the wrong type, root node != V-Control");
		logIT(LOG_ERR,string);
		xmlFreeDoc(doc);
		return(0);
	}
	...
	if ((xc=xmlXIncludeProcessFlags(doc,XML_PARSE_XINCLUDE|XML_PARSE_NOXINCNODE))==0) {
		sprintf(string,"Kein XInclude durchgefuehrt");
		logIT(LOG_WARNING,string);
	}
	else if (xc<0) {
		sprintf(string,"Fehler bei XInclude");
		logIT(LOG_ERR,string);
		return(0);
	}
	else {
	...
	cur=cur->children;
	while (cur) {
		sprintf(string,"XML: (%d) Node::Name=%s Type:%d Content=%s",cur->line,cur->name,cur->type,cur->content);
		logIT(LOG_INFO,string);
		if (xmlIsBlankNode(cur))  {
			cur=cur->next;
			continue;
		}
		if (strstr(cur->name,"unix"))  {
		...
		if (strstr(cur->name,"extern"))  {
			cur=cur->children;
			if (cur && strstr(cur->name,"vito"))  {
				prevPtr=cur;
		...
		else if (strstr(cur->name,"protocols"))  {
		...
		else if (strstr(cur->name,"units"))  {
			if (!(TuPtr=parseUnit(cur->children)))
				return(0);
			cur=cur->next; /* gleiche Ebene wie unix, also next */
			continue;
		}
		else if (strstr(cur->name,"commands"))  {
		...
		else if (strstr(cur->name,"devices"))  {
			if (!(TdevPtr=parseDevice(cur->children,TprotoPtr)))
				return(0);
			(cur->next) ? (cur=cur->next) : (cur=prevPtr->next); 
			continue;
		}
		else if (strstr(cur->name,"config")) { 
	...
	while(cPtr) {
		...
		while(dPtr) {
			...
			/* den kennen wir nicht und kopieren die Daten */
				sprintf(string,"Kopiere Kommando %s nach Device %s",cPtr->name,dPtr->id);
				logIT(LOG_INFO,string);
	...
	if (!(TcfgPtr->devPtr=getDeviceNode(TdevPtr,TcfgPtr->devID))) {
			sprintf(string,"Device %s nicht definiert\n",TcfgPtr->devID);
			logIT(LOG_ERR,string);
			return(0);
	}
